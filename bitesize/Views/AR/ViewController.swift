...
//        }
        ...
        if let result = arView.raycast(from: tapLocation, allowing: .estimatedPlane, alignment: .any).first {
            ...
            if lastNode != nil {
                ...
                lengthLabel.text = String(format: "Distance: %.1f cm", totalDistance * 100)
...
//            if let previousPosition = positions.last {
//                let distance = lastPosition!.distance(to: previousPosition)
//                totalDistance -= distance
//                lengthLabel.text = String(format: "Distance: %.1f cm", totalDistance * 100)
//            }
//            lengthLabel.textColor = .red
//            lastNode.removeFromScene()
//        }
    }
...
//            totalDistance = 0.0
    ...
    func model(for classification: ARMeshClassification) -> ModelEntity {
        ...
        let extrusionDepth: Float = 0.005
...
//        let textMesh = MeshResource.generateText(classification.description, extrusionDepth: Float(lineHeight * 0.1), font: font)
        let textMesh = MeshResource.generateText(classification.description, extrusionDepth: extrusionDepth, font: font)
        var textMaterial = SimpleMaterial(color: classification.color, isMetallic: true)
        
        //stroke
        let strokeMesh = MeshResource.generateText(classification.description, extrusionDepth: 0.02, font: font) // Adjust extrusion depth for the stroke
        let strokeMaterial = SimpleMaterial(color: .black, isMetallic: true)
        
        
        let model = ModelEntity(mesh: textMesh, materials: [textMaterial, strokeMaterial])
